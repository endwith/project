...
public class OOSUtil {
    private static final Logger logger = LoggerFactory.getLogger(OOSUtil.class);
    @Value("${endpoint}")
...
//解析结果
     ...
     *  //阿里云上传图形验证码
     *  */
    public String  uploadPictureCode(InputStream inputStream,String pictureCode) {
        String fileName= null;
...
//初始化OSSClient
        ...
        try {
//文件名
            fileName= "http://"+bucketName+"."+ endpoint+"/"+pictureCode + ".jpg";
//上传文件 (上传文件流的形式)
            PutObjectResult putResult = ossClient.putObject(bucketName, pictureCode+".jpg", inputStream);
//解析结果
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("上传阿里云OSS服务器异常." + e.getMessage(), e);
