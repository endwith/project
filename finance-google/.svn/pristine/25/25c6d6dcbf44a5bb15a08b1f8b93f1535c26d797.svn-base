...
private RedisTemplate redisTemplate;
    ...
    private void startScheduler4(Scheduler scheduler) throws SchedulerException {
        JobDetail jobDetail = JobBuilder.newJob(ReturnInterestJob.class).withIdentity("job4", "jobGroup4").build();
        String day = "";
        if(redisTemplate.hasKey("googleJudgeHolidayStatus")) {
            if((int)redisTemplate.opsForValue().get("googleJudgeHolidayStatus")==1){
                day = String.valueOf((int) redisTemplate.opsForValue().get("googleJudgeHoliday")+1);
                //假日不打款，先往后拖一天，拖到不是节假日后就变成？每天都运行
            }else {
                day = "?";
            }
        }
        String cron = "1 0 0 "+day+" * ? *";
        CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule(cron);
        CronTrigger trigger = TriggerBuilder.newTrigger()
    ...
    private void startScheduler5(Scheduler scheduler) throws SchedulerException {
        JobDetail jobDetail = JobBuilder.newJob(ReturnAccountAllJob.class).withIdentity("job5", "jobGroup5").build();
        String day = "";
        if(redisTemplate.hasKey("googleJudgeHolidayStatus")) {
            if((int)redisTemplate.opsForValue().get("googleJudgeHolidayStatus")==1){
                day = String.valueOf((int) redisTemplate.opsForValue().get("googleJudgeHoliday")+1);
